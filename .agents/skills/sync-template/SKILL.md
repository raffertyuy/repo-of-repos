...
| Path | What |
|------|------|
| `.agents/skills/*/SKILL.md` | All skill definitions (canonical) |
| `.claude/skills/*/SKILL.md` | Skill stubs (frontmatter + `@` pointer to canonical) |
| `.claude/agents/*.md` | Agent definitions (read by Claude Code and Copilot) |
| `.claude/prompt-snippets/*.md` | Shared prompt snippets |
| `.claude/rules/*.md` | Auto-applied rules (read by Claude Code and VS Code Copilot) |
| `CLAUDE.md` | One-line `@AGENTS.md` pointer (template-owned, copy as-is) |
...
1. Scan local `.agents/skills/*/SKILL.md` and `.claude/agents/*.md` for files that have `origin: template` in their frontmatter
...
4. Show the user a list of these orphaned template files and ask for confirmation before deleting:
   ...
   - .agents/skills/list-tasks/SKILL.md
   Delete these? [y/N]
   ```
5. If confirmed, delete the files and their parent directories (only if the directory contains no other files). When removing a skill, delete BOTH the canonical `.agents/skills/<name>/` directory AND its stub `.claude/skills/<name>/` directory
...
AGENTS.md (the canonical instruction file — `CLAUDE.md` is just a one-line `@AGENTS.md` pointer) is the most complex merge target. Live projects customize it heavily — different persona, project-specific architecture, custom coding standards, additional sections. The template evolves its framework sections independently.

**Pre-0.6.0 workspaces**: if the local workspace still has full instructions in `CLAUDE.md` and no `AGENTS.md`, migrate first — treat the local `CLAUDE.md` content as the local `AGENTS.md` for merging purposes, write the merged result to `AGENTS.md`, and replace `CLAUDE.md` with the one-line pointer.
...
- Per-Repo Instructions (table of `AGENTS.md` / `CLAUDE.md` / `copilot-instructions.md`)
...
- Cross-Tool Compatibility (table + rules)
- Self-Improvement
...
**Project-only sections** — Anything in the local AGENTS.md that has no counterpart in the upstream template. Common examples:
...
**First-line / title:** The local AGENTS.md may have a different title or opening line (e.g., `# aws-mk0` instead of `# AGENTS.md`). Preserve the local title.
...
**Three-way merge procedure:** Same as AGENTS.md (see above).
...
| File | Action |
|------|--------|
| `.agents/skills/pull-all-repos/SKILL.md` | Copied from upstream |
| `AGENTS.md` | Merged (added "Git vs Local Entries" section) |
| `.gitignore` | Merged (switched to per-repo entries) |
| ... | ... |

### New files
- `.agents/skills/sync-template/SKILL.md` (+ stub in `.claude/skills/sync-template/`)

### Removed files
- `.agents/skills/create-task/` *(template skill removed in v0.5.0)*
- `.agents/skills/list-tasks/` *(template skill removed in v0.5.0)*
...
- When copying a skill, keep the canonical file and its stub consistent: the stub's frontmatter must match the canonical frontmatter, followed only by `@../../../.agents/skills/<name>/SKILL.md`
- **Never touch** `repos/repos.md`, `repos/*/`, `_plans/*.plan.md` (user plans), `.claude/settings*.json`, `.mcp.json`, `.codex/config.toml`
